package uncover

import (
	"context"
	"fmt"
	"github.com/projectdiscovery/gologger"
	errorutil "github.com/projectdiscovery/utils/errors"
	stringsutil "github.com/projectdiscovery/utils/strings"
	"github.com/wjlin0/pathScan/pkg/projectdiscovery/uncover/sources"
	"github.com/wjlin0/pathScan/pkg/projectdiscovery/uncover/sources/agent/anubis"
	"github.com/wjlin0/pathScan/pkg/projectdiscovery/uncover/sources/agent/binary"
	"github.com/wjlin0/pathScan/pkg/projectdiscovery/uncover/sources/agent/bing"
	"github.com/wjlin0/pathScan/pkg/projectdiscovery/uncover/sources/agent/censys"
	"github.com/wjlin0/pathScan/pkg/projectdiscovery/uncover/sources/agent/chinaz"
	"github.com/wjlin0/pathScan/pkg/projectdiscovery/uncover/sources/agent/criminalip"
	"github.com/wjlin0/pathScan/pkg/projectdiscovery/uncover/sources/agent/fofa"
	"github.com/wjlin0/pathScan/pkg/projectdiscovery/uncover/sources/agent/google"
	"github.com/wjlin0/pathScan/pkg/projectdiscovery/uncover/sources/agent/hunter"
	"github.com/wjlin0/pathScan/pkg/projectdiscovery/uncover/sources/agent/hunterhow"
	"github.com/wjlin0/pathScan/pkg/projectdiscovery/uncover/sources/agent/ip138"
	"github.com/wjlin0/pathScan/pkg/projectdiscovery/uncover/sources/agent/netlas"
	"github.com/wjlin0/pathScan/pkg/projectdiscovery/uncover/sources/agent/publicwww"
	"github.com/wjlin0/pathScan/pkg/projectdiscovery/uncover/sources/agent/qianxun"
	"github.com/wjlin0/pathScan/pkg/projectdiscovery/uncover/sources/agent/quake"
	"github.com/wjlin0/pathScan/pkg/projectdiscovery/uncover/sources/agent/rapiddns"
	"github.com/wjlin0/pathScan/pkg/projectdiscovery/uncover/sources/agent/shodan"
	"github.com/wjlin0/pathScan/pkg/projectdiscovery/uncover/sources/agent/shodanidb"
	"github.com/wjlin0/pathScan/pkg/projectdiscovery/uncover/sources/agent/sitedossier"
	"github.com/wjlin0/pathScan/pkg/projectdiscovery/uncover/sources/agent/zoomeye"
	"github.com/wjlin0/pathScan/pkg/writer"
	"os"
	"strings"
	"sync"
	"time"
)

var DefaultChannelBuffSize = 32

type Options struct {
	Agents   []string // Uncover Agents to use
	Queries  []string // Queries to pass to Agents
	Limit    int
	MaxRetry int
	Timeout  int
	// Note these ratelimits are used as fallback in case agent
	// ratelimit is not available in DefaultRateLimits
	RateLimit     uint          // default 30 req
	RateLimitUnit time.Duration // default unit
	Proxy         string
	ProxyAuth     string
}

// Service handler of all uncover Agents
type Service struct {
	Options  *Options
	Agents   []sources.Agent
	Session  *sources.Session
	Provider *sources.Provider
	Keys     sources.Keys
}

// New creates new uncover service instance
func New(opts *Options) (*Service, error) {
	s := &Service{Agents: []sources.Agent{}, Options: opts}
	for _, v := range opts.Agents {
		switch v {
		case "shodan":
			s.Agents = append(s.Agents, &shodan.Agent{})
		case "censys":
			s.Agents = append(s.Agents, &censys.Agent{})
		case "fofa":
			s.Agents = append(s.Agents, &fofa.Agent{})
		case "shodan-idb":
			s.Agents = append(s.Agents, &shodanidb.Agent{})
		case "quake":
			s.Agents = append(s.Agents, &quake.Agent{})
		case "hunter":
			s.Agents = append(s.Agents, &hunter.Agent{})
		case "zoomeye":
			s.Agents = append(s.Agents, &zoomeye.Agent{})
		case "netlas":
			s.Agents = append(s.Agents, &netlas.Agent{})
		case "criminalip":
			s.Agents = append(s.Agents, &criminalip.Agent{})
		case "publicwww":
			s.Agents = append(s.Agents, &publicwww.Agent{})
		case "hunterhow":
			s.Agents = append(s.Agents, &hunterhow.Agent{})
		case "binary":
			s.Agents = append(s.Agents, &binary.Agent{})
		case "bing":
			s.Agents = append(s.Agents, &bing.Agent{})
		case "anubis":
			s.Agents = append(s.Agents, &anubis.Agent{})
		case "chinaz":
			s.Agents = append(s.Agents, &chinaz.Agent{})
		case "google":
			s.Agents = append(s.Agents, &google.Agent{})
		case "ip138":
			s.Agents = append(s.Agents, &ip138.Agent{})
		case "qianxun":
			s.Agents = append(s.Agents, &qianxun.Agent{})
		case "rapiddns":
			s.Agents = append(s.Agents, &rapiddns.Agent{})
		case "sitedossier":
			s.Agents = append(s.Agents, &sitedossier.Agent{})
		}

	}
	s.Provider = sources.NewProvider()
	s.Keys = s.Provider.GetKeys()

	if opts.RateLimit == 0 {
		opts.RateLimit = 30
	}
	if opts.RateLimitUnit == 0 {
		opts.RateLimitUnit = time.Minute
	}

	var err error
	s.Session, err = sources.NewSession(&s.Keys, opts.MaxRetry, opts.Timeout, int(opts.RateLimit), opts.Agents, opts.RateLimitUnit, opts.Proxy, opts.ProxyAuth)
	if err != nil {
		return nil, err
	}
	return s, nil
}

func (s *Service) Execute(ctx context.Context) (<-chan sources.Result, error) {
	// unlikely but as a precaution to handle random panics check all types
	if err := s.nilCheck(); err != nil {
		return nil, err
	}
	switch {
	case len(s.Agents) == 0:
		return nil, errorutil.NewWithTag("uncover", "no agent/source specified")
	case !s.hasAnyAnonymousProvider() && !s.Provider.HasKeys():
		return nil, errorutil.NewWithTag("uncover", "agents %v requires keys but no keys were found", s.Options.Agents)
	}

	megaChan := make(chan sources.Result, DefaultChannelBuffSize)
	// iterate and run all uncover
	wg := &sync.WaitGroup{}
	for _, q := range s.Options.Queries {
	agentLabel:
		for _, agent := range s.Agents {
			keys := s.Provider.GetKeys()
			if keys.Empty() && !(stringsutil.EqualFoldAny(agent.Name(), AnonymousAgents()...)) {
				gologger.Debug().Msgf(agent.Name(), "agent given but keys not found")
				continue agentLabel
			}
			defaultQuery := sources.DefaultQuery(q, agent.Name())
			gologger.Debug().Msgf("Request parameter is: %s - %s", agent.Name(), defaultQuery)
			ch, err := agent.Query(s.Session, &sources.Query{
				Query: defaultQuery,
				Limit: s.Options.Limit,
			})
			if err != nil {
				gologger.Debug().Msgf("%s\n", err)
				continue agentLabel
			}
			wg.Add(1)
			go func(source, relay chan sources.Result, ctx context.Context) {
				defer wg.Done()
				for {
					select {
					case <-ctx.Done():
						return
					case res, ok := <-source:
						res.Timestamp = time.Now().Unix()
						if !ok {
							return
						}
						relay <- res
					}
				}
			}(ch, megaChan, ctx)
		}
	}

	// close channel when all uncover return
	go func(wg *sync.WaitGroup, megaChan chan sources.Result) {
		wg.Wait()
		defer close(megaChan)

	}(wg, megaChan)

	return megaChan, nil
}

// ExecuteWithCallback ExecuteWithWriters writes output to writer along with stdout
func (s *Service) ExecuteWithCallback(ctx context.Context, callback func(result sources.Result)) error {
	ch, err := s.Execute(ctx)
	if err != nil {
		return err
	}
	if callback == nil {
		return errorutil.NewWithTag("uncover", "result callback cannot be nil")
	}
	for {
		select {
		case <-ctx.Done():
			return nil
		case result, ok := <-ch:
			if !ok {
				return nil
			}
			callback(result)
		}
	}
}

// AllAgents returns all supported uncover Agents
func AllAgents() []string {
	return []string{
		"shodan", "censys", "fofa", "quake", "hunter", "zoomeye", "netlas", "criminalip", "publicwww", "hunterhow", "binary",
		"shodan-idb", "anubis", "bing", "chinaz", "google", "ip138", "qianxun", "rapiddns", "sitedossier",
	}
}
func UncoverAgents() []string {
	return []string{
		"shodan", "censys", "fofa", "quake", "hunter", "zoomeye", "netlas", "criminalip", "publicwww", "hunterhow", "binary",
	}
}

func AnonymousAgents() []string {
	return []string{
		"shodan-idb", "anubis", "bing", "chinaz", "google", "ip138", "qianxun", "rapiddns", "sitedossier",
	}
}

func (s *Service) nilCheck() error {
	if s.Provider == nil {
		return errorutil.NewWithTag("uncover", "provider cannot be nil")
	}
	if s.Options == nil {
		return errorutil.NewWithTag("uncover", "options cannot be nil")
	}
	if s.Session == nil {
		return errorutil.NewWithTag("uncover", "session cannot be nil")
	}
	return nil
}

func (s *Service) hasAnyAnonymousProvider() bool {
	return func() bool {
		for _, str := range AnonymousAgents() {
			if stringsutil.EqualFoldAny(str, s.Options.Agents...) {
				return true
			}
		}
		return false
	}()
}

func GetTarget(limit int, field string, csv bool, output string, engine, query []string, proxy, auth string) (<-chan string, error) {
	var (
		err          error
		outputWriter *writer.OutputWriter
	)
	opts := Options{
		Agents:    engine,
		Queries:   query,
		Limit:     limit,
		MaxRetry:  2,
		Timeout:   20,
		Proxy:     proxy,
		ProxyAuth: auth,
	}

	outputWriter, err = writer.NewOutputWriter()
	if err != nil {
		return nil, err
	}
	if output != "" {
		switch {
		case csv:
			csvWriter, err := writer.NewCSVWriter(output, sources.Result{})
			if err != nil {
				return nil, err
			}
			outputWriter.AddWriters(csvWriter)
		default:
			create, err := os.Create(output)
			if err != nil {
				return nil, err
			}
			outputWriter.AddWriters(create)
		}
	}
	u, err := New(&opts)
	if err != nil {
		return nil, err
	}
	ret := make(chan string)

	ch, err := u.Execute(context.Background())
	go func(ch <-chan sources.Result) {
		defer close(ret)
		for result := range ch {
			switch {
			case result.Error != nil:
				gologger.Warning().Msgf("Request %s sending error: %s", result.Source, result.Error)
			default:
				replacer := strings.NewReplacer("ip", result.IP, "host", result.Host,
					"port", fmt.Sprint(result.Port),
				)
				outData := replacer.Replace(field)
				ret <- outData
				switch {
				case csv:
					toString, err := writer.CSVToString(result)
					if err != nil {
						continue
					}
					outputWriter.Write(toString)
				default:
					outputWriter.WriteString(outData)
				}
			}

		}
	}(ch)

	return ret, err
}
